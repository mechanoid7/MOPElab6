...
for num_of_coef in num_of_coef_list:  # ймовірний перехід до рівняння регресії з урахуванням ефекту взаємодії та квадратичних членів
    ...
    print("Fp:", Fp)
    print("Ft:", Ft)
    if Fp > Ft:  # если Fp > Ft, то выполняется этот блок
        print("Рівняння регресії не адекватно оригіналу при рівні значимості 0,05")
        if num_of_coef == num_of_coef_list[0]:  # если впервые Fp > Ft, тогда главный цикл на строке 120
                                                # продолжает выполнятся, берётся следующее количество коэффициентов - 8
            print("Переходимо до рівняння з урахуванням ефекту взаємодії.")
        elif num_of_coef == num_of_coef_list[1]:  # если второй раз Fp > Ft, тогда главный цикл на строке 120
                                                  # продолжает выполнятся, берётся следующее количество коэффициентов - 11
            print("Переходимо до рівняння з урахуванням квадратичних членів.")
            # на строках 205/230 и 255/261 считаются дополнительные коэффициенты, в зависимости от типа уравнения
            # а на строках 274/277/281 віберается вівод уравнения регрессии 'y'
    else:
